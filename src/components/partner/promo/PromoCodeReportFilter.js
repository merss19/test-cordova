...
import Pickaday from '../../../assets/js/libs/pickaday'
...
const periodSelectors = [
    {id: 1, label: "Произвольный", value: "custom"},
    {id: 2, label: "Сегодня", value: "day"},
    {id: 3, label: "Вчера", value: "yesterday"},
    {id: 4, label: "Неделя", value: "week"},
    {id: 5, label: "Месяц", value: "month"},
    {id: 6, label: "Квартал", value: "quarter"},
    {id: 7, label: "Год", value: "year"}];
...
class PromoCodeReportFilter extends Component {
    ...
    constructor(params) {
        ...
        this.state = {period: periodSelectors[1].value,
            promoCodes: [],
    ...
    handleAddPromoTag(tag, state) {
        tag.name.split(",")
            .forEach((name)=>{
               state.promoCodes.push({name:name})
       });
        this.onRefreshData();
    };
    ...
    onRefreshData() {
        this.onChangeOption({
            ...
            promoCodes: this.state.promoCodes.length > 0 ? this.state.promoCodes.map((item)=> { return item.name  }) : null

        }, (result) => {
            const { promoCodesSuggestions : suggestions} = this.state;
            this.setState({promoCodesSuggestions: this.enrichPromoSuggestions(result, suggestions)});
        })
    }
        ...
        this.periodStartsAtPicker = new Pickaday(
            {
                ...
                onSelect: function () {
                    self.setState({period: periodSelectors[0].value});
                    self.onRefreshData()
                }
            });
    ...
    createPeriodRadios() {
        const {period} = this.state, setPeriod  = this.setPeriod;
        return periodSelectors.map(function (item) {
            return (<label key={item.id} className="radio-button-label">
                <div>
                <input className="radio-button-control" value={item.value} name="period" type="radio"
                       checked={period == item.value} onChange={setPeriod}/><span className="radio-button-text">{item.label}</span></div>
        ...
        return (
            <div className="grid">
                <div className="1/2--pocket 1/2--lap 1/2--desk grid__cell">
                    ...
                    <div className="promo-report-filter-group">
                        ...
                        <div className="1/2--pocket 1/2--lap 1/2--desk grid__cell">
                            <div className="input input--box">
                                <input disabled id="periodEndsAt" name="periodEndsAt"
                                       ...
                                       type="text" className="input__field input__field--date"/>
                            </div>
                        </div>
                    </div>
                </div>
...
export default PromoCodeReportFilter;