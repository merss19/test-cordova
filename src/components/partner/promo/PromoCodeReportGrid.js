...
const PROMO_METRICS_GRAPH_OPTIONS = {
    "Traffic" : {"axis":['Дата', 'Трафик'], "chartType":"LineChart","columns":[{"label":"time","type":"date"},{"label":"Количество посещений","type":"number"}],"rows":[],"options":{"legend":'none',"hAxis":{format: 'dd MMM yyyy', "title":"Дата"},"vAxis":{"title":"Количество посещений"}},"width":"100%"},
    "Registration" : {"axis":['Дата', 'Регистраций'],"chartType":"LineChart","columns":[{"label":"time","type":"date"},{"label":"Количество регистраций","type":"number"}],"rows":[],"options":{"legend":'none',"hAxis":{format: 'dd MMM yyyy', "title":"Дата"},"vAxis":{"title":"Количество регистраций"}},"width":"100%"},
    "PaidRegistration" : {"axis":['Дата', 'Регистраций c оплатами'], "chartType":"LineChart","columns":[{"label":"time","type":"date"},{"label":"Количество посещений","type":"number"}],"rows":[],"options":{"legend":'none',"hAxis":{format: 'dd MMM yyyy', "title":"Дата"},"vAxis":{"title":"Количество рег. с оплатами"}},"width":"100%"},
    "PaidAmount" : {"axis":['Дата', 'Сумма оплат'], "chartType":"LineChart","columns":[{"label":"time","type":"date"},{"label":"Сумма оплат","type":"number"}],"rows":[],"options":{"legend":'none',"hAxis":{format: 'dd MMM yyyy', "title":"Дата"},"vAxis":{"title":"Сумма оплат"}},"width":"100%"}
};
...
class PromoCodeReportGrid extends Component {
    ...
    constructor(params) {
        ...
        this.state = { metricValue:"Traffic", tableData: [], summary: {}, graphData:[], graphOptions: PROMO_METRICS_GRAPH_OPTIONS["Traffic"]};
    }
    ...
    static getPromoCodeReport(filter) {
        ...
        return fetch(`${api}/partner/accessLogPartner-promoCodeReport`, {
            ...
            method: 'POST',
            body: JSON.stringify({authToken: token ? token : cookie.load('token'), data: {...filter}})
        });
    }
        ...
        return fetch(`${api}/partner/accessLogPartner-promoMetricReport`, {
            ...
            body: JSON.stringify({authToken: token ? token : cookie.load('token'), data: {...filter
                ...
                , timeUnit : moment(filter.periodEndsAt, "YYYY-MM-DDTHH:mm:ss").diff(moment(filter.periodStartsAt, "YYYY-MM-DDTHH:mm:ss"),'Days') <= 2
                    ? 'Hours'
                    : moment(filter.periodEndsAt, "YYYY-MM-DDTHH:mm:ss").diff(moment(filter.periodStartsAt, "YYYY-MM-DDTHH:mm:ss"),'Days') <= 180 ? 'Days'
                    : "Weeks"}})
        });
    }
    ...
    render() {
        return (
            <div className="grid promo-report">
                <div className="1/1--pocket 1/1--lap 1/1--desk grid__cell">
                    ...
                    <div className="1/1--pocket 1/1--lap 1/2--desk grid__cell">
                        <div className="promo-chart">
                            <div className="promo-chart-metric">
                                <select className="select select__field" value={this.state.metricValue} onChange={this.handleMetricChange}>
                    ...
                    <div className="1/1--pocket 1/1--lap 1/1--desk grid__cell">
                        <div className="base-table promo-report-table">
                            <TableView
...
export default PromoCodeReportGrid