...
const enConfig = {
    ...
    year: year => year,
    month: month => ['Январь', 'Февраль', 'Март', 'Апрель', 'Май', 'Июнь', 'Июль', 'Август', 'Сентябрь', 'Октябрь', 'Ноябрь', 'Декабрь'][month],
    ...
    dateLabel: (fullDate, type, use24) => {
        ...
        case 'date':
	        ...
	        if (mm < 10) mm = '0' + mm;
	        const result = `${enConfig.date(date.getDate())}-${mm}-${enConfig.year(date.getFullYear())}`
            return result
        default:
            return ''
        }
